from custom_items.data_fetching import dataset_constructor, dom_aware_dataset_constructor, dataset_constructor_signfi, dom_aware_dataset_constructor_signfi
...
from custom_items.losses import dom_aware_decoupl_nt_xent_loss
...
if not args.transfer:
    ...
    if args.dataset == 'widar3':
        construct_func = dom_aware_dataset_constructor
    elif args.dataset == 'signfi':
        construct_func = dom_aware_dataset_constructor_signfi
    else:
        raise ValueError("Unknown dataset: {}".format(args.dataset))
...
pretrain_model.compile(optimizer=[tf.keras.optimizers.Adam(learning_rate=args.learning_rate), tfa.optimizers.SGDW(weight_decay=1e-6, learning_rate=args.learning_rate * 5, momentum=0.95, nesterov=True)],
                       sim_loss_fn=dom_aware_decoupl_nt_xent_loss, class_loss_fn=tf.keras.losses.CategoricalCrossentropy(), tau=args.tau,
                       metrics=[[tf.keras.metrics.CategoricalAccuracy(), MultiClassPrecision(num_classes=class_nr, average='weighted'),
                                ...
                                tfa.metrics.CohenKappa(num_classes=class_nr, sparse_labels=False)], []], awareness_type='batch', dset_type=args.dataset, transfer=args.transfer)
# ------------------------------------------------------------------------------------------------------------------
