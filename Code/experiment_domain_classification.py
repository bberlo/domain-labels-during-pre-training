...
from custom_items.losses import decoupl_nt_xent_loss, sec_approx_categ_ce
from custom_items.metrics import MultiClassPrecision, MultiClassRecall
from custom_items.pretrain_models import AdvClassPretrainModel
from custom_items.finetune_model import FinetuneModel
from custom_items.callbacks import WeightRestoreCallback, DomainLossWeightWarmup
...
cnn_extractor = MobileNetV2Backbone(input_shape=input_shape, backbone_name=args.model_name + "_backbone").get_model()
...
pretrain_model = AdvClassPretrainModel(inp, [proj, enc], name=args.model_name)

inp_disc = tf.keras.layers.Input(shape=(enc._type_spec.shape[-1] * 2,))

disc = tf.keras.layers.Dense(600, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(inp_disc)
disc = tf.keras.layers.Activation(tf.keras.activations.relu)(disc)
disc = tf.keras.layers.Dense(450, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(disc)
disc = tf.keras.layers.Activation(tf.keras.activations.relu)(disc)
disc = tf.keras.layers.Dense(300, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(disc)
disc = tf.keras.layers.Activation(tf.keras.activations.relu)(disc)
disc = tf.keras.layers.Dense(domain_nr, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(disc)
disc = tf.keras.layers.Activation(tf.keras.activations.softmax)(disc)

disc_model = tf.keras.Model(inp_disc, disc, name=args.model_name + "_discriminator")
...
pretrain_model.compile(optimizer=tfa.optimizers.SGDW(weight_decay=1e-6, learning_rate=args.learning_rate * 5, momentum=0.95, nesterov=True),
                       domain_class_loss_fn=sec_approx_categ_ce, sim_loss_fn=decoupl_nt_xent_loss, discriminator=disc_model,
                       loss_weights=[tf.keras.backend.variable(1.0), tf.keras.backend.variable(-1.0 * args.domain_loss_weight)], tau=args.tau, domain_class_loss_type='single-label')
# ------------------------------------------------------------------------------------------------------------------
...
pre_callback_objects = [
    ...
    # DomainLossWeightWarmup(slope=0.001, max_epochs=25, ringbuf_size=6, verbose=1, domain_loss_weight=args.domain_loss_weight, slope_bound_scale=30)
]
