...
# ------------------------------------------------- End-to-end supervised pretraining ----------------------------------
inp = tf.keras.layers.Input(shape=input_shape)
cnn_extractor = MobileNetV2Backbone(backbone_name=args.model_name + "_backbone_pre-train", input_shape=input_shape).get_model()
...
enc = tf.keras.layers.Dense(trans_pretr_class_nr, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(enc)
enc = tf.keras.layers.Activation(tf.keras.activations.softmax)(enc)

endtoend_model = tf.keras.models.Model(inp, enc, name=args.model_name + "_pre-train")
endtoend_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.learning_rate), loss=tf.keras.losses.CategoricalCrossentropy(),
                       metrics=[tf.keras.metrics.CategoricalAccuracy(), MultiClassPrecision(num_classes=trans_pretr_class_nr, average='weighted'),
                                MultiClassRecall(num_classes=trans_pretr_class_nr, average='weighted'), tfa.metrics.F1Score(num_classes=trans_pretr_class_nr, average='weighted'),
                                tfa.metrics.CohenKappa(num_classes=trans_pretr_class_nr, sparse_labels=False)])
# -------------------------------------------------------------------------------------------------------------------
...
endtoend_train_history = endtoend_model.fit(x=pre_train_set, epochs=args.epoch_size, steps_per_epoch=len(train_instance_indices) // args.batch_size,
                                       verbose=2, callbacks=callback_objects, validation_data=pre_val_set,
...
finetune_model.layers[1].set_weights(endtoend_model.layers[1].get_weights())
...
finetune_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.learning_rate), loss=tf.keras.losses.CategoricalCrossentropy(),
                       metrics=[tf.keras.metrics.CategoricalAccuracy(), MultiClassPrecision(num_classes=class_nr, average='weighted'),
                                ...
                                tfa.metrics.CohenKappa(num_classes=class_nr, sparse_labels=False)], dset_type=args.dataset, transfer=True, end_to_end=True)
# -------------------------------------------------------------------------------------------------------------------
...
_, acc, precision, recall, f_score, kappa_score = finetune_model.evaluate(x=test_set, steps=len(test_instances) // args.batch_size, verbose=2)
metrics_frame = pd.DataFrame(data=[[acc, precision, recall, f_score, kappa_score]], columns=['A', 'P', 'R', 'F', 'CK'])

results_frame = pd.concat([endtoend_history_frame, fine_history_frame, metrics_frame], axis=1)
