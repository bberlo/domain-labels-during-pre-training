...
parser.add_argument('-m_n', '--model_name', help='<Required> Model name: domain_class, no_domain_class, multilabel_domain_class, multilabel_domain_class_2', required=True)
parser.add_argument('-t', '--type', help='<Required> Experiment type: in-domain or domain-leave-out', required=True)
...
if args.data_type == 'ampphase':
    if args.dataset == 'signfi' and args.domain_type == 'user':
        ...
        dataset_path = r'Datasets/signfi-domain-leave-out-dataset-raw-environment.hdf5'
        domain_loss_weight = 0.01
...
if args.model_name == 'domain_class':
    ...
    f_name = 'experiment_supervised_no_domain_classification.py'
elif args.model_name == 'domain_aware_batch':
    f_name = 'experiment_dom_batch_no_domain_classification.py'
elif args.model_name == 'domain_aware_filter_denom':
    f_name = 'experiment_dom_filter_denom_no_domain_classification.py'
elif args.model_name == 'domain_aware_batch_alt_flow':
    f_name = 'experiment_dom_alt_flow_batch_no_domain_classification.py'
elif args.model_name == 'domain_aware_filter_denom_alt_flow':
    f_name = 'experiment_dom_alt_flow_filter_denom_no_domain_classification.py'
else:
...
if args.type == 'in-domain':
    if args.dataset == 'signfi':
        raise ValueError("Signfi does not allow in-domain splitting because there is no domain "
                         "factor overlap in the dataset")
...
elif args.type == 'domain-leave-out':
    ...
    if args.dataset == 'signfi':
        ...
        elif args.domain_type == 'position':
            CUR_DOMAIN_FACTOR = 2
            CUR_DOMAIN_FACTOR_TOTAL_NR = 5
        elif args.domain_type == 'orientation':
            CUR_DOMAIN_FACTOR = 3
            CUR_DOMAIN_FACTOR_TOTAL_NR = 5
        else:
            raise ValueError("Unknown domain type. Allowed values: user, position, orientation")
...
if args.crossval_split:
    ...
    test_types_list = [test_types_list[args.crossval_split]]
...
for split_nr, test_types in zip(split_nrs, test_types_list):
    ...
    elif args.model_name == "no_domain_class" or args.model_name == "domain_aware_batch" or \
            args.model_name == "domain_aware_filter_denom" or args.model_name == "domain_aware_batch_alt_flow" or \
            args.model_name == "domain_aware_filter_denom_alt_flow":
