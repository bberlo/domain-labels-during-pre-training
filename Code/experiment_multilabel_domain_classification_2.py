...
from custom_items.losses import decoupl_nt_xent_loss, ml_xent_loss_2
...
disc = tf.keras.layers.Dense(450, use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(enc)
...
disc = tf.keras.layers.Activation(tf.keras.activations.relu)(disc)
disc = tf.keras.layers.Dense(math.ceil(domain_nr / 2), use_bias=False, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=0.33, mode='fan_in', distribution='uniform', seed=args.seed))(disc)

pretrain_model = AdvClassPretrainModel(inp, [proj, disc], name=args.model_name)
pretrain_model.compile(optimizer=tfa.optimizers.SGDW(weight_decay=1e-6, learning_rate=args.learning_rate * 5, momentum=0.95, nesterov=True),
                       domain_class_loss_fn=ml_xent_loss_2, sim_loss_fn=decoupl_nt_xent_loss, discriminator=None, domain_class_loss_type='multi-label',
                       loss_weights=[tf.keras.backend.variable(1.0), tf.keras.backend.variable(-1.0 * args.domain_loss_weight)], tau=args.tau)
# ------------------------------------------------------------------------------------------------------------------
...
pre_callback_objects = [
    ...
    # DomainLossWeightWarmup(slope=0.005, max_epochs=25, ringbuf_size=6, verbose=1, domain_loss_weight=args.domain_loss_weight, slope_bound_scale=15)
]
...
results_frame.to_csv("results/{}_results_dfn_{}_dt_{}_ds_{}_cvs_{}_{}.csv".format(args.model_name + "_2", args.domain_factor_name, args.data_type, args.dataset, str(args.crossval_split), datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
